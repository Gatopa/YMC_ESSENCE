...
(function(){
  ...
  function update(){
    ...
    if (diff <= 0){
      dd.textContent = hh.textContent = mm.textContent = "00"; return;
    }
...
// Murciélagos tamaño normal (no tocamos transform para no romper animación)
(function(){
  ...
  for(var i=0;i<N;i++){
    ...
    // ligeramente distintos anchos (no tocamos transform)
    el.style.width = Math.round(rand(14, 22)) + "vw";

    wrap.appendChild(el);
  }

  // quitar del DOM cuando termine todo
